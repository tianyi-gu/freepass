...
- Native successful login, failed-login retry, consent decline/enable/revoke, secure-session restoration after process restart, and reading a private synthetic document have also passed. The temporary-account deletion continuation, iPad and packaging checks are still in progress. Several initial automation failures were incorrect selectors or retained Keychain sessions; tests now sign out through FreePass instead of wiping the simulator Keychain.
...
6. **Finish App Store release deliberately.** Merge [code PR #5](https://github.com/tianyi-gu/freepass/pull/5) and privacy-policy PRs, attach the new OpenAI build, refresh screenshots/review notes if needed, verify the reviewer account, then submit. Apple has not approved these changes. Never run an old script that attaches build 10 and submits automatically.
