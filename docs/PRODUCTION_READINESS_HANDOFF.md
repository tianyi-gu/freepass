...
**Release decision: not ready for public launch yet.** The OpenAI implementation, database hardening and content corrections are implemented and deployed where applicable. The final candidate is EAS build 13 (`97817a46-ed17-4e27-802d-af9e52d36c23`), built successfully, uploaded for TestFlight, processed by Apple as **VALID**, and attached to ASC version 1.0. The IPA uses the iOS 26 SDK, includes 12 privacy manifests, calls the server endpoint, and contains none of the provider-key values. Build 11 completed but was superseded; build 12 was canceled for a final timeout fix. Operations, directory review, and physical-device acceptance remain gates. This replaces the older build-10/Gemini handoff; do not follow the old automated review-submission instructions.
...
1. **Accept email on the release build and monitor delivery.** The free Gmail sender `freepassnotifications@gmail.com` is now configured with SMS two-step verification and a separate SMTP app password; no authenticator app was enrolled. All 12 live signup/resend/recovery checks passed, and the synthetic account was deleted. Confirmation remains required; the saved limit is 20 emails/hour. Complete physical-device, other-recipient-domain and time-expired-code acceptance. Gmail is a limited-volume sender with delivery restrictions; the owner must monitor failures and migrate to a transactional provider when needed. See [email setup](audit/EMAIL_SETUP.md).
...
6. **Finish App Store release deliberately.** Merge [code PR #5](https://github.com/tianyi-gu/freepass/pull/5) and [privacy-policy PR #2](https://github.com/tianyi-gu/freepass-privacy/pull/2). Build 13, screenshots, privacy declarations and reviewer notes are already prepared; recheck reviewer access immediately before submission after the gates above pass. Apple has not approved these changes. Never run an old script that attaches build 10 and submits automatically.
