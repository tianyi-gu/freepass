...
**Release decision: not ready for public launch yet.** The OpenAI implementation, database hardening and content corrections are implemented and deployed where applicable. The final candidate is EAS build 13 (`97817a46-ed17-4e27-802d-af9e52d36c23`), built successfully, uploaded for TestFlight, processed by Apple as **VALID**, and attached to ASC version 1.0. The IPA uses the iOS 26 SDK, includes 12 privacy manifests, calls the server endpoint, and contains none of the provider-key values. Build 11 completed but was superseded; build 12 was canceled for a final timeout fix. Email delivery, operations, directory review, and physical-device acceptance remain gates. This replaces the older build-10/Gemini handoff; do not follow the old automated review-submission instructions.
